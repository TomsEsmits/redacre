...
class Red_Acre_Gutenberg {
	...
	public function register_block_scripts() {
		foreach( glob( get_template_directory() . '/blocks/**/block.json' ) as $block ) :
			...
			if ( isset( $data->redacreVendorScripts ) ) :
				if ( is_array( $data->redacreVendorScripts ) ) :
					foreach ( $data->redacreVendorScripts as $script ) :
						$vendorScripts[] = $script;
					endforeach;
				else:
					$vendorScripts[] = $data->redacreVendorScripts;
				endif;
			endif;

			if ( isset( $data->redacreVendorStyles ) ) :
				if ( is_array( $data->redacreVendorStyles ) ) :
					foreach ( $data->redacreVendorStyles as $style ) :
						$vendorStyles[] = $style;
					endforeach;
				else:
					$vendorStyles[] = $data->redacreVendorStyles;
				endif;
			endif;
			...
			if ( has_block( $data->name ) ) :
				...
				if ( isset( $data->redacreScripts ) && $data->redacreScripts === true ) :
	...
	public static function get_preview_image( $block_image, $block_name )
	{
		...
		if ( file_exists( $image ) ) :
			...
			$output = '<div class="block-editor-inserter__preview-content-missing">' . __( 'No preview available', 'redacre' ) . '</div>';
		endif;
		return apply_filters('redacre_block_preview_image', $output, $block_image, $block_name );
	}
}

new Red_Acre_Gutenberg();